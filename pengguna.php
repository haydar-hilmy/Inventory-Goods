...
$query_pengguna = mysqli_query($con, "SELECT * FROM master");

if(isset($_POST['tambah'])){
    if(addPengguna($_POST) > 0){
        echo "<script>alert('Berhasil menambah pengguna!');window.location = 'pengguna.php';</script>";
    } else {
        echo "<script>alert('Oops! gagal menambah pengguna :(');</script>";
    }
}

if(isset($_GET['cari'])){
    $pencarian = $_GET['pengguna'];
    $query_pengguna = mysqli_query($con, "SELECT * FROM master WHERE nama_pengguna LIKE '%$pencarian%'");
}
    ...
    <div class="nav-x-content">
        ...
        <div class="content">
            ...
            <form method="get" class="custom-sort">
                    <input name="pengguna" id="search-good" class="search-input" placeholder="Cari Pengguna" type="text">
                ...
                <table class="good-table" cellspacing="0px">
                    ...
                    <tbody id="list-good">
                        <?php $i = 1; foreach($query_pengguna as $key) { ?>
                        <tr>
                            <td><?php echo $i++; ?></td>
                            <td><?php echo $key["nama_pengguna"]; ?></td>
                            <td><?php echo $key["email"]; ?></td>
                            <td><?php echo $key["jabatan"]; ?></td>
                            <td><a href='edit_data/edit_pengguna.php?id=<?php echo $key["id"]; ?>' id="edit-btn"><i class='fa-regular fa-pen-to-square'></i></a> <a href='edit_data/delete_pengguna.php?id=<?php echo $key["id"]; ?>'><i class='fa-regular fa-trash-can'></i></a></td>
                        </tr>
                        <?php } ?>
                    </tbody>
                </table>
    ...
    <div class="modal-form" id="form-add-wrap">
        <form method="post" class="form-add" id="form-add">
            ...
            <div class="input-div">
                ...
                <input class="" type="text" name="jabatan" required><br>

                <label for="password">Password</label>
                <input class="" type="password" name="password" required><br>

                <button type="submit" name="tambah" class="btn-add-form">Tambah</button>
            </div>
