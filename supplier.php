...
if (isset($_POST['tambah'])) {
    if (addSupplier($_POST) > 0) {
        echo "<script>alert('Berhasil menambah supplier!');window.location = 'supplier.php';</script>";
    } else {
        echo "<script>alert('Oops! gagal menambah supplier :(');window.location = 'supplier.php';</script>";
    }
}

if(isset($_GET['cari'])){
    $pencarian = $_GET['supplier'];
    $query_supplier = mysqli_query($con, "SELECT * FROM supplier WHERE nama_supplier LIKE '%$pencarian%'");
}
        ...
        <div class="content">
            ...
            <div class="table-wrap">
                <form method="get" class="custom-sort">
                    <input name="supplier" id="search-good" class="search-input" placeholder="Cari Supplier" type="text">
                ...
                <table class="good-table" cellspacing="0px">
                    ...
                    <tbody id="list-good">
                        <?php $i = 1;
                        foreach ($query_supplier as $key) { ?>
                            <tr>
                                <td><?php echo $i++; ?></td>
                                <td><?php echo "SPL_" . $key['id']; ?></td>
                                <td><?php echo $key['nama_supplier']; ?></td>
                                <td><?php echo $key['alamat']; ?></td>
                                <td><?php echo $key['email_supplier']; ?></td>
                                <td><?php echo $key['no_telp']; ?></td>
                                <td><a href='edit_data/edit_supplier.php?id=<?php echo $key["id"]; ?>' id="edit-btn"><i class='fa-regular fa-pen-to-square'></i></a> <a href='edit_data/delete_supplier.php?id=<?php echo $key["id"]; ?>'><i class='fa-regular fa-trash-can'></i></a></td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
    ...
    <div class="modal-form" id="form-add-wrap">
        <form method="post" class="form-add" id="form-add">
            ...
            <div class="input-div">
                ...
                <input class="" type="email" name="email" required><br>

                <label for="telp">No. Telp</label>
                <input class="" type="number" name="telp" min="1" required><br>

                <button type="submit" class="btn-add-form" name="tambah">Tambah</button>
            </div>
